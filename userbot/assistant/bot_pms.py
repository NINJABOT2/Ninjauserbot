...
@Ninja.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"rules")))
...
@Ninja.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"users")))
...
@Ninja.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"depy")))
async def help(event):
    if event.query.user_id == bot.uid:
        await event.answer("This Is Not For U My Master", cache_time=0, alert=True)
    else:
        await event.client.send_message(
            event.chat_id,
            message="You Can Deploy/Install NinjaBot In Heroku By Following Steps Bellow, You Can See Some Quick Guides On Support Channel Or On Your Own Assistant Bot. \nThank You For Contacting Me.",
            link_preview=False,
            buttons=[
                [
                ...
                [
                    Button.url(
                        "Github Repo ❓", "https://github.com/NINJABOT2/NINJABOT2.0"
                    ),
                ],
            ],
        )
...
@Ninja.bot_cmd(incoming=True, func=lambda e: e.is_private)
...
@Ninja.bot_cmd(pattern="^/uinfo$", from_users=Config.OWNER_ID)
async def bot_start(event):
    reply_to = await reply_id(event)
    if not reply_to:
...
async def send_flood_alert(user_) -> None:
    ...
    if FloodConfig.ALERT and (user_.id in FloodConfig.ALERT.keys()):
        ...
        try:
            ...
            if BOTLOG:
                await Ninja.tgbot.send_message(
                    BOTLOG_CHATID,
    ...
    if found:
        if flood_count >= FloodConfig.AUTOBAN:
            if user_.id in Config.SUDO_USERS:
                ...
                if BOTLOG:
                    await Ninja.tgbot.send_message(BOTLOG_CHATID, sudo_spam)
            else:
        ...
        if not fa_id:
            return
        try:
            msg_ = await Ninja.tgbot.get_messages(BOTLOG_CHATID, fa_id)
        ...
        if BOTLOG:
            fa_msg = await Ninja.tgbot.send_message(
                BOTLOG_CHATID,
                flood_msg,
                buttons=buttons,
            )
        try:
            chat = await Ninja.tgbot.get_entity(BOTLOG_CHATID)
            await Ninja.tgbot.send_message(
                Config.OWNER_ID,
            ...
            if BOTLOG:
                await Ninja.tgbot.send_message(BOTLOG_CHATID, "**Unblock your bot !**")
    if FloodConfig.ALERT[user_.id].get("fa_id") is None and fa_msg:
        FloodConfig.ALERT[user_.id]["fa_id"] = fa_msg.id


@Ninja.tgbot.on(CallbackQuery(data=re.compile(b"bot_pm_ban_([0-9]+)")))
@check_owner
async def bot_pm_ban_cb(c_q: CallbackQuery):
    user_id = int(c_q.pattern_match.group(1))
    try:
        user = await Ninja.get_entity(user_id)
    except Exception as e:
...
@Ninja.tgbot.on(CallbackQuery(data=re.compile(b"toggle_bot-antiflood_off$")))
@check_owner
...
@Ninja.bot_cmd(incoming=True, func=lambda e: e.is_private)
@Ninja.bot_cmd(edited=True, func=lambda e: e.is_private)
