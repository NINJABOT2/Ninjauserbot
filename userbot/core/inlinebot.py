...
from ..plugins import ALIVE_NAME, USERID, Ninja_grp, mention
...
@Ninja.tgbot.on(CallbackQuery(data=re.compile(b"help_k_minu")))
@check_owner
...
def main_menu():
    ...
    buttons = [
        ...
        [
            ...
            Button.url("Support 🇮🇳", "https://t.me/NinjaBot_AI"),
        ],
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        ...
        if query.startswith("**NinjaBot"):
            ...
            if ALIVE_PIC is None:
                I_IMG = "https://telegra.ph/file/c5a74e1817b97a6cd3fe4.jpg"
            else:
            ...
            result = builder.article(
                title="Repository",
                text=f"**⚜ Ninjaary Af NinjaBot ⚜**",
                buttons=[
                    [Button.url("♥️ Tutorial ♥", "https://youtu.be/CH_KO1wim2o")],
                    [Button.url("📍 𝚁𝚎𝚙𝚘 📍", "https://github.com/NINJABOT2/NINJABOT2.0")],
                    [
                        Button.url(
                            "💞 Deploy 💞",
                            "https://heroku.com/deploy?template=https://github.com/NINJABOT2/NINJABOT2.0",
                        )
                    ],
                ],
            )
            ...
            try:
                ...
                try:
                    ...
                    if u.username:
                        ...
                        Ninja = f"[{u.first_name}](tg://user?id={u.id})"
                    u = int(u.id)
                except ValueError:
                    # ValueError: Could not find the input entity
                    Ninja = f"[user](tg://user?id={u})"
            except ValueError:
                # if u is username
                try:
            ...
            result = builder.article(
                title="Troll Message",
                text=f"🌹 Only This : {Ninja} cannot access this message !",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            if jsondata:
            ...
            result = builder.article(
                ...
                text=f"🔒 A whisper message to {Ninja}, Only he/she can open it.",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            if jsondata:
            ...
            if oso is None:
                help_pic = "https://telegra.ph/file/c5a74e1817b97a6cd3fe4.jpg"
            else:
            ...
            if oso == "OFF":
                result = builder.article(
                    title="NinjaBot© Help",
                    description="Help menu for NinjaBot",
                    text=_result[0],
                    buttons=_result[1],
                    link_preview=False,
                )
            ...
            if PM_PIC:
                Ninja = [x for x in PM_PIC.split()]
                PIC = list(Ninja)
                Ninja_IMG = random.choice(PIC)
            else:
                Ninja_IMG = None
            query = gvarstatus("pmpermit_text")
            if Ninja_IMG and Ninja_IMG.endswith((".jpg", ".jpeg", ".png")):
                result = builder.photo(
                    Ninja_IMG,
                    # title="Alive Ninja",
                    text=query,
            ...
            buttons = [
                (
                    Button.url("Source code", "https://github.com/NINJABOT2/NINJABOT2.0"),
                    Button.url(
                        "Deploy",
                        "https://dashboard.heroku.com/new?button-url=https%3A%2F%2Fgithub.com%2FNINJAHACKER01%2FNinjaBOT&template=https%3A%2F%2Fgithub.com%2FNINJAHACKER01%2FNinjaBOT",
                    ),
                )
            ]
            ALV_PIC = "https://telegra.ph/file/c5a74e1817b97a6cd3fe4.jpg"
            markup = event.client.build_reply_markup(buttons)
            photo = types.InputWebDocument(
            ...
            result = types.InputBotInlineResult(
                ...
                type="photo",
                title=f"ñïñjåẞø†",
                description=f"ñïñjåẞø†\nhttps://t.me/NinjaBot_OP",
                url="https://github.com/NINJABOT2/NINJABOT2.0",
                thumb=photo,
                content=photo,
                send_message=types.InputBotInlineMessageMediaAuto(
                    reply_markup=markup, message=text, entities=msg_entities
...
@Ninja.tgbot.on(CallbackQuery(data=re.compile(b"clise")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    buttons = [
    ...
    await event.edit(
        f"📜 Menu Provider Has Been Closed\n\n🔰 Bot Of : {mention}\n\n             [©️ñïñjåẞø†](https://t.me/NinjaBot_OP)",
        buttons=buttons,
        link_preview=False,
    )


@Ninja.tgbot.on(CallbackQuery(data=re.compile(b"check")))
...
@Ninja.tgbot.on(CallbackQuery(data=re.compile(b"(.*)_menu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
...
@Ninja.tgbot.on(CallbackQuery(data=re.compile(rb"mainmenu")))
@check_owner
async def on_plug_in_callback_query_handler(event):
    _result = main_menu()
