...
from . import Ninjaversion
from .core.logger import logging
from .core.session import Ninja, tgbot

LOGS = logging.getLogger("NinjaUserBot")
Ninja_PIC = "https://telegra.ph/file/c5a74e1817b97a6cd3fe4.jpg"
...
@Ninja.tgbot.on(events.NewMessage(pattern="/start", func=lambda x: x.is_group))
...
async def help(event):
    ...
    if event.query.user_id is not bot.uid:
        ...
        await tgbot.send_message(
            event.chat_id,
            message=f"Hey, I am your {bot_id}'s assistant bot.\nI Am Here To Help U \n\nPowered By [NinjaBot](https://t.me/NinjaBot_OP)",
            buttons=[
                [
                    Button.url(" Support ", "https://t.me/NinjaBot_OP"),
                    Button.url(" Updates ", "https://t.me/NinjaBot_AI"),
                ],
                [
...
async def rel(event):
    if event.query.user_id == bot.uid:
        await event.answer("ShutDown ñïñjåẞø†...", cache_time=0, alert=True)
        if BOTLOG:
...
@Ninja.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"res_tart")))
async def res_ttart(event):
    if event.query.user_id == bot.uid:
        await event.answer("Restarting Please Wait 4 min... ", cache_time=0, alert=True)
        if BOTLOG:
            Ninja = await event.client.send_message(
                BOTLOG_CHATID, "# RESTART \n" "Bot Restarted"
            )
        try:
            await Ninja.disconnect()
...
@Ninja.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"setvar")))
...
@Ninja.tgbot.on(events.callbackquery.CallbackQuery(data=re.compile(b"Ahack")))
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Banning all members. Thanks For using NinjaBoy Bot", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(i + "\n\nThanks For using NinjaBoy Bot", buttons=keyboard)
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Joined the Channel/Group Thanks For using NinjaBoy Bot", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Deleted the Channel/Group Thanks For using NinjaBoyBot.", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        if i:
            await event.reply(
                "User don't have two step thats why now two step is `NinjaBoy Bot Is best` you can login now\n\nThanks For using NinjaBoy Bot.",
                buttons=keyboard,
            )
        else:
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        try:
            ...
            await event.respond(
                "SEND THIS ERROR TO - @Ninja_Userbot\n**LOGS**\n" + str(e)
            )
