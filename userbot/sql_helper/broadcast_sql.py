...
class NinjaBroadcast(BASE):
    __tablename__ = "Ninjabroadcast"
    ...
    def __repr__(self):
        return "<Ninja Broadcast channels '%s' for %s>" % (
            self.group_id,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, NinjaBroadcast)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


NinjaBroadcast.__table__.create(checkfirst=True)

NinjaBROADCAST_INSERTION_LOCK = threading.RLock()
...
def add_to_broadcastlist(keywoard, group_id):
    with NinjaBROADCAST_INSERTION_LOCK:
        broadcast_group = NinjaBroadcast(keywoard, str(group_id))
...
def rm_from_broadcastlist(keywoard, group_id):
    with NinjaBROADCAST_INSERTION_LOCK:
        broadcast_group = SESSION.query(NinjaBroadcast).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_broadcastlist(keywoard):
    with NinjaBROADCAST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(NinjaBroadcast.keywoard)
            .filter(NinjaBroadcast.keywoard == keywoard)
            .delete()
        )
...
def get_broadcastlist_chats():
    try:
        chats = SESSION.query(NinjaBroadcast.keywoard).distinct().all()
...
def num_broadcastlist():
    try:
        return SESSION.query(NinjaBroadcast).count()
    finally:
        SESSION.close()
...
def num_broadcastlist_chats():
    try:
        return SESSION.query(func.count(distinct(NinjaBroadcast.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_broadcastlists():
    try:
        ...
        all_groups = SESSION.query(NinjaBroadcast).all()
        for x in all_groups:
