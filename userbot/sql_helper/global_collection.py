...
class Ninja_GlobalCollection(BASE):
    __tablename__ = "Ninja_globalcollection"
    ...
    def __repr__(self):
        return "<Ninja Global Collection lists '%s' for %s>" % (
            self.contents,
            self.keywoard,
        )
    ...
    def __eq__(self, other):
        return bool(
            isinstance(other, Ninja_GlobalCollection)
            and self.keywoard == other.keywoard
            and self.contents == other.contents
        )


Ninja_GlobalCollection.__table__.create(checkfirst=True)

Ninja_GLOBALCOLLECTION = threading.RLock()
...
def add_to_collectionlist(keywoard, contents):
    with Ninja_GLOBALCOLLECTION:
        keyword_items = Ninja_GlobalCollection(keywoard, tuple(contents))
...
def rm_from_collectionlist(keywoard, contents):
    with Ninja_GLOBALCOLLECTION:
        keyword_items = SESSION.query(Ninja_GlobalCollection).get(
...
def del_keyword_collectionlist(keywoard):
    with Ninja_GLOBALCOLLECTION:
        keyword_items = (
            SESSION.query(Ninja_GlobalCollection.keywoard)
            .filter(Ninja_GlobalCollection.keywoard == keywoard)
            .delete()
        )
...
def get_collectionlist_items():
    try:
        chats = SESSION.query(Ninja_GlobalCollection.keywoard).distinct().all()
...
def num_collectionlist():
    try:
        return SESSION.query(Ninja_GlobalCollection).count()
    finally:
        SESSION.close()
...
def num_collectionlist_items():
    try:
        return SESSION.query(
            func.count(distinct(Ninja_GlobalCollection.keywoard))
        ).scalar()
    finally:
        SESSION.close()
...
def __load_item_collectionlists():
    try:
        ...
        all_groups = SESSION.query(Ninja_GlobalCollection).all()
        for x in all_groups:
