...
class NinjaGloballist(BASE):
    __tablename__ = "Ninjaglobal_list"
    ...
    def __repr__(self):
        return "<Ninja global values '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, NinjaGloballist)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


NinjaGloballist.__table__.create(checkfirst=True)

NinjaGLOBALLIST_INSERTION_LOCK = threading.RLock()
...
def add_to_list(keywoard, group_id):
    with NinjaGLOBALLIST_INSERTION_LOCK:
        broadcast_group = NinjaGloballist(keywoard, str(group_id))
...
def rm_from_list(keywoard, group_id):
    with NinjaGLOBALLIST_INSERTION_LOCK:
        broadcast_group = SESSION.query(NinjaGloballist).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_list(keywoard):
    with NinjaGLOBALLIST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(NinjaGloballist.keywoard)
            .filter(NinjaGloballist.keywoard == keywoard)
            .delete()
        )
...
def get_list_keywords():
    try:
        chats = SESSION.query(NinjaGloballist.keywoard).distinct().all()
...
def num_list():
    try:
        return SESSION.query(NinjaGloballist).count()
    finally:
        SESSION.close()
...
def num_list_keywords():
    try:
        return SESSION.query(func.count(distinct(NinjaGloballist.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_lists():
    try:
        ...
        all_groups = SESSION.query(NinjaGloballist).all()
        for x in all_groups:
