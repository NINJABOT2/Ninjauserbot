...
async def animator(media, mainevent, textevent):
    ...
    NinjaOp = await mainevent.client.download_media(media, Config.TEMP_DIR)
    await textevent.edit("__🎞Converting into Animated sticker..__")
    await runcmd(
        f"ffmpeg -ss 00:00:00 -to 00:00:02.900 -i {NinjaOp} -vf scale={w}:{h} -c:v libvpx-vp9 -crf 30 -b:v 560k -maxrate 560k -bufsize 256k -an animate.webm"
    )  # pain
    os.remove(NinjaOp)
...
async def hide_inlinebot(borg, bot_name, text, chat_id, reply_to_id, c_lick=0):
    ...
    Ninja = await sticcers[c_lick].click("me", hide_via=True)
    if Ninja:
        await borg.send_file(int(chat_id), Ninja, reply_to=reply_to_id)
        await Ninja.delete()
...
async def waifutxt(text, chat_id, reply_to_id, bot):
    ...
    Ninja = await sticcers[0].click("me", hide_via=True)
    if Ninja:
        await bot.send_file(int(chat_id), Ninja, reply_to=reply_to_id)
        await Ninja.delete()
