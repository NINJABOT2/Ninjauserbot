...
async def d_paste(message, extension=None):
    ...
    """
    siteurl = "http://Ninjabin.up.railway.app/documents"
    data = {"content": message}
    try:
        response = requests.post(url=siteurl, data=json.dumps(data), headers=headers)
    except Exception as e:
    ...
    if response.ok:
        response = response.json()
        purl = (
            f"http://Ninjabin.up.railway.app/{response['key']}.{extension}"
            if extension
            else f"http://Ninjabin.up.railway.app/{response['key']}"
        )
        return {
            "url": purl,
            "raw": f"http://Ninjabin.up.railway.app/raw/{response['key']}",
            "bin": "Dog",
        }
