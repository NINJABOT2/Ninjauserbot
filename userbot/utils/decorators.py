    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.HANDLER) == 2:
                Ninjareg = "^" + Config.HANDLER
                reg = Config.HANDLER[1]
            elif len(Config.HANDLER) == 1:
                Ninjareg = "^\\" + Config.HANDLER
                reg = Config.HANDLER
            args["pattern"] = re.compile(Ninjareg + pattern)
...
def register(**args):
    ...
    def decorator(func):
        if not disable_edited:
            Ninja.add_event_handler(func, MessageEdited(**args))
        Ninja.add_event_handler(func, NewMessage(**args))
        try:
